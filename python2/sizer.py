...
class Sizer:
    def __init__(self, sizer_directory):
    ...
    def add_images(self, images_directory):
        images = os.listdir(images_directory)
        self.grain_count+= len(images)
        for image in images:
            image_ = cv2.imread(images_directory+image)
            self.__sum += image_.shape[0] 
            
    def size_up(self, report_directory):
